        ...
        protected virtual void DrawLabels(SKCanvas canvas, string[] texts, SKPoint[] points, SKRect[] sizes, SKColor[] colors, Orientation orientation, bool isTop, SKSize itemSize, float height)
        {
            if (points.Length > 0)
            {
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    {
                        DrawHelper.DrawLabel(canvas, orientation, isTop ? YPositionBehavior.UpToElementHeight : YPositionBehavior.None, entry.ValueLabelFitToBar ? itemSize : SKSize.Empty, points[i], colors[i], sizes[i], texts[i], LabelTextSize, Typeface, Margin);
                    }
                }
            }
        }
